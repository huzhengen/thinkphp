...
//		$this->assign('orderList', $orderList);
	...
	public function listsdztoday(){
		...
		$datebegin = date("Y-m-d",time());
		$dateend = date("Y-m-d 23:59:59",time());
		$dzlist = \think\Db::name('order')->where('del', '0')->where('daozhen', '<>', 0)->where('dztime', 'between', [$datebegin, $dateend])->count();//总到诊
		$orderList = \think\Db::name('order')->where('del', 0)->where('daozhen', '<>', 0)->where('dztime', 'between', [$datebegin, $dateend])->order('djtime', 'desc')->paginate(15);
		if(request()->isGet()){
			$btime = input('begintime');
			$begintime = $btime . ' 00:00:00';
			$begintimeend = $btime . ' 23:59:59';
			...
			// 按时间搜索
			if($btime && !$etime){ // 开始时间，没有结束时间
				$orderList = \think\Db::name('order')
					->where('del', 0)
					->where('daozhen', '<>', 0)
					->where('dztime','between', [$btime,$begintimeend])
					->order('djtime', 'desc')->paginate(15);
				$dzlist = \think\Db::name('order')
					->where('del', 0)
					->where('daozhen', '<>', 0)
					->where('dztime','between', [$btime,$begintimeend])->count();
			}
			if(!$btime && $etime){ // 没有开始时间，有结束时间
				dump(1);
				$orderList = \think\Db::name('order')
					->where('del', 0)
					->where('daozhen', '<>', 0)
				...
				$dzlist = \think\Db::name('order')
					->where('del', 0)
					->where('daozhen', '<>', 0)
					->where('dztime','between', [$etime,$endtime])->count();
			}
			if($btime && $etime){ // 开始时间、结束时间都有
				$orderList = \think\Db::name('order')
					->where('del', 0)
					->where('daozhen', '<>', 0)
					->where('dztime','between', [$btime,$endtime])
					->order('djtime', 'desc')->paginate(15);
				$dzlist =  \think\Db::name('order')
					->where('del', 0)
					->where('daozhen', '<>', 0)
					->where('dztime','between', [$btime,$endtime])->count();
			}
			// 按时间+客服搜索
			if($btime && !$etime && $kefu){ // 有开始时间、客服，没有结束时间
				$kefu = '%' . $kefu . '%';
				$orderList = \think\Db::name('order')
					->where('del', 0)
					->where('daozhen', '<>', 0)
				...
				$dzlist = \think\Db::name('order')
					...
					->where('daozhen', '<>', 0)
					->where('dztime','between', [$btime,$begintimeend])
					->where('kefu', 'like', $kefu)->count();
			}
			if(!$btime && $etime && $kefu){ // 没有开始时间，有结束时间和客服
				$kefu = '%' . $kefu . '%';
				$orderList = \think\Db::name('order')
					->where('del', 0)
					->where('daozhen', '<>', 0)
			...
			if($btime && $etime && $kefu){ // 有开始时间、结束时间、客服，三项
				$kefu = '%' . $kefu . '%';
				$orderList = \think\Db::name('order')
					->where('del', 0)
					->where('daozhen', '<>', 0)
			...
			if(!$btime && !$etime && $kefu){ // 没有开始结束时间，有客服
				$kefu = '%' . $kefu . '%';
				$orderList = \think\Db::name('order')
					->where('del', 0)
					->where('daozhen', '<>', 0)
					->where('dztime', 'between', [$datebegin, $dateend])
					->where('kefu', 'like', $kefu)
					->order('djtime', 'desc')->paginate(15);
				$dzlist = \think\Db::name('order')
					->where('del', 0)
			...
			if($ddzt){
				...
				$dzlist = \think\Db::name('order')
					->where('del', 0)
					->where('daozhen', $ddzt)
					->where('dztime', 'between', [$datebegin, $dateend])->count();
			}
			if($kefu && $ddzt){
				$kefu = '%' . $kefu . '%';
				$orderList = \think\Db::name('order')
			...
			if($disease){
				...
				$dzlist = \think\Db::name('order')
					->where('del', 0)
					->where('daozhen', '<>', 0)
					->where('disease', 'like', $disease)->count();
			}
			if($name){
				$name = '%' . $name . '%';
				$orderList = \think\Db::name('order')
			...
			if($beizhu){
				...
				$dzlist = \think\Db::name('order')
					->where('del', 0)
					->where('daozhen', '<>', 0)
					->where('beizhu',  'like', $beizhu)->count();
			}
			if($tel){
				$tel = '%' . $tel . '%';
				$orderList = \think\Db::name('order')
			...
			if($weixin){
				...
				$dzlist = \think\Db::name('order')
					->where('del', 0)
					->where('daozhen', '<>', 0)
					->where('weixin', 'like', $weixin)->count();
			}
			if($qq){
				$qq = '%' . $qq . '%';
				$orderList = \think\Db::name('order')
