...
if($phone){
	...
	$sql4 = "SELECT * FROM tp5_0416_antaibao WHERE black='1'";
	$result4 = mysql_query($sql4)  or die(mysql_error());
	while($resultrow4 = mysql_fetch_row($result4)){
		//电话在黑名单里面
		if($resultrow4[1] == $phone){
			$sql42 = "SELECT COUNT(`ip`) FROM tp5_0416_antaibao WHERE ip='$ip' AND dianhua='$phone' AND tjtime < '$time' AND tjtime > '$timeprev'";
			$result42 = mysql_query($sql42)  or die(mysql_error());
			$result42=mysql_fetch_row($result42);
			$sql43 = "SELECT COUNT(`dianhua`) FROM tp5_0416_antaibao WHERE dianhua='$phone' AND tjtime like '$timenohlike'";
			...
			if($result42[0]>=3){
				...
				if($result43[0]>=3){
					...
					$sql41 = "INSERT INTO tp5_0416_antaibao (dianhua,padd,ip,tjtime,url,shebei,ipadd,black) VALUES ('".$phone."', '".$padd."', '".$ip."', '".$time."', '".$url."', '".$device."', '".$ipadd."', 1)";
		...
		//IP在黑名单里面
		if($resultrow4[6] == $ip){
			$sql42 = "SELECT COUNT(`ip`) FROM tp5_0416_antaibao WHERE ip='$ip' AND tjtime < '$time' AND tjtime > '$timeprev'";
			$result42 = mysql_query($sql42)  or die(mysql_error());
			$result42=mysql_fetch_row($result42);
			$sql43 = "SELECT COUNT(`dianhua`) FROM tp5_0416_antaibao WHERE dianhua='$phone' AND tjtime like '$timenohlike'";
			$result43 = mysql_query($sql43)  or die(mysql_error());
		...
		//URL在黑名单里面
		if($resultrow4[8] == $ip){
			$sql42 = "SELECT COUNT(`ip`) FROM tp5_0416_antaibao WHERE ip='$ip' AND url='$url' AND tjtime < '$time' AND tjtime > '$timeprev'";
			$result42 = mysql_query($sql42)  or die(mysql_error());
			$result42=mysql_fetch_row($result42);
			$sql43 = "SELECT COUNT(`dianhua`) FROM tp5_0416_antaibao WHERE dianhua='$phone' AND tjtime like '$timenohlike'";
			$result43 = mysql_query($sql43)  or die(mysql_error());
	...
	$sql3 = "SELECT COUNT(`dianhua`) FROM tp5_0416_antaibao WHERE dianhua='$phone' AND tjtime like '$timenohlike'";
	$result3 = mysql_query($sql3)  or die(mysql_error());
	$result3=mysql_fetch_row($result3);
	$sql5 = "SELECT COUNT(`dianhua`) FROM tp5_0416_antaibao WHERE dianhua='$phone' AND tjtime < '$time' AND tjtime > '$timeprev5min'";
	...
	if($result2[0]>=3){
		...
		if($result5[0]>=1){
			...
			if($result3[0]>=3){
				...
				$sql1 = "INSERT INTO tp5_0416_antaibao (dianhua,padd,ip,tjtime,url,shebei,ipadd) VALUES ('".$phone."', '".$padd."', '".$ip."', '".$time."', '".$url."', '".$device."', '".$ipadd."')";
